...
def get_stored_sessions(stage: str, file_ids, num_episodes):
    ...
    if not online_episodes_list or len(online_episodes_list[0]) == 0:
        raise ValueError(f"No episodes found in {data_dir}\n filenames {filenames}")
    
    # check if episodes contain information
    if len(online_episodes_list[0][0]['user_signals']) == 0:
        ...
        assert len(online_episodes_list[0][0]['user_signals']) > 0, 'Could not load episode files'
