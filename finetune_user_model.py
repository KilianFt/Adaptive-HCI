...
def main(model: LightningModule, user_hash, config: configs.BaseConfig) -> LightningModule:
    ...
    print(f"Found {len(episode_filenames)} episodes in {online_data_dir}")
    print("Full path:", os.path.abspath(online_data_dir))
    # os pwd

    print(check_output("pwd", shell=True).decode())
    print(check_output("ls -l", shell=True).decode())
    print(check_output(f"ls -l {online_data_dir}", shell=True).decode())
    print("Done")
